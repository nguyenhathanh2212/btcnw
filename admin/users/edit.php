...
<?php require_once $_SERVER['DOCUMENT_ROOT']."/functions/checkuser.php" ?>
	...
	<div class="content">
			...
			<?php
				...
				$resultUser = $DB->select($queryUser)[0];
			...
			<?php
				if (isset($_POST['edit'])) {
					...
					if (md5($cfPassword) != $passwordUser) {
						...
						if ($DB->update('users', $data, "id_user='{$idUser}'")) {
							header("location:index.php?msg=Sửa thành công");
							$session->set('msgSuccess', 'Cập nhật người dùng thành công');
						} else {
							$session->set('msgError', 'Cập nhật người dùng thất bại');
						}
